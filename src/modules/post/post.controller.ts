...
export class PostController {
  ...
  @ApiParam({ name: 'commentId', description: 'The ID of the comment to reply to', type: Number })
  @ApiResponse({
    status: 201,
  ...
  @ApiParam({ name: 'commentId', description: 'The ID of the comment to update', type: Number })
  @ApiResponse({
    status: 200,
  ...
  @ApiParam({ name: 'commentId', description: 'The ID of the comment to delete', type: Number })
  ...
  @Get(':postId/comments')
  @ApiOperation({ summary: 'Get all comments for a post with pagination and search' })
  ...
  @ApiResponse({ status: 200, description: 'A list of comments.', type: [CommentDto] })
  async getComments(
    @Param('postId') postId: number,
    @Query() getCommentsDto: GetCommentsDto,
  ): Promise<CommentDto[]> {
    getCommentsDto.postId = postId;
    getCommentsDto.parentId = undefined; // Ensure only top-level comments are fetched
    const comments = await this.queryBus.execute(new GetCommentsQuery(getCommentsDto));

    return comments.map(comment => comment.props);
  }

  @Get(':postId/comments/:commentId/replies')
  @ApiOperation({ summary: 'Get all replies for a comment with pagination and search' })
  @ApiParam({
    name: 'postId',
    description: 'The ID of the post the comment belongs to',
    type: Number,
  })
  @ApiParam({ name: 'commentId', description: 'The ID of the parent comment', type: Number })
  @ApiQuery({ name: 'page', required: false, type: Number, description: 'Page number', example: 1 })
  @ApiQuery({
    name: 'limit',
    required: false,
  ...
  @ApiQuery({
    ...
    required: false,
    type: String,
    description: 'Search term for reply content',
    example: 'My reply',
  })
  @ApiQuery({
    name: 'sort',
  ...
  @ApiResponse({ status: 200, description: 'A list of replies.', type: [CommentDto] })
  async getCommentReplies(
    ...
    getCommentsDto.postId = postId;
    getCommentsDto.parentId = commentId;
    const comments = await this.queryBus.execute(new GetCommentsQuery(getCommentsDto));
